...
## R8.5-UC11 Amendment Lock Management Screen

### Feature: Amendment Lock Management Screen

**As an** Administrator
**I want to** view and manage all active amendment locks via UI
**So that** I can force-release stale locks without database access

**Background**
Administrators need a dedicated screen to manage amendment locks when users abandon sessions.

#### 37. Scenario: View all active amendment locks (Happy Path)
**Given** the Administrator is logged in with "TF_ADMIN" role
**When** the Administrator navigates to the **"Amendment Lock Management"** screen
**Then** the system should display a list of all active locks (LcAmendmentLock)
**And** for each lock show: LC Number, Locked By User, Locked At, Lock Expiry, Lock Type
**And** the system should allow filtering by: LC Number, User ID, Lock Status

#### 38. Scenario: Filter locks by LC Number (Edge Case - Search)
**Given** the Administrator is on the Lock Management screen
**When** the Administrator enters LC Number "LC-2026-001" in the filter
**Then** the system should display only locks matching that LC
**And** the system should show "No active locks" if no matches found

#### 39. Scenario: Force release lock via UI (Happy Path)
**Given** the Administrator sees a stale lock on LC "LC-2026-001"
**When** the Administrator clicks the **"Release Lock"** button
**And** the Administrator confirms the action
**And** the Administrator enters reason: "User session expired"
**Then** the system should call forceRelease#AmendmentLock service
**And** the lock should be removed from the list
**And** the system should display success message: "Lock released successfully"
**And** the system should create audit trail entry

#### 40. Scenario: Prevent non-admin from accessing lock screen (Edge Case - Security)
**Given** a regular user (CSR) is logged in
**When** the CSR attempts to access the Lock Management screen
**Then** the system should return "Access Denied" error
**And** the screen should not be visible in navigation

#### 41. Scenario: Lock expires automatically (Edge Case - Scheduled Job)
**Given** a lock has lockExpiry set to "2026-03-17T10:30:00"
**When** the scheduled job runs after expiry time
**Then** the system should automatically release expired locks
**And** the lock should be removed from the active locks list

## Business Rules & Validation
1. **Admin Only:** Only users with TF_ADMIN role can access Lock Management screen.
2. **Audit Trail:** All force releases must be logged with reason and user ID.
3. **Notification:** Original lock holder should be notified if they reconnect.
4. **Auto-Expiry:** Locks should automatically expire after lockExpiry timestamp.
